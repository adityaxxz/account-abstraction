...
contract ZkMinimalAccountTest is Test {
    ...
    uint256 constant PAY_AMOUNT = 1e18; 


    function setUp() public {
        minimalAccount = new ZkMinimalAccount();
        //* transfer ownership so that we can sign the txn with ANVIL_DEFAULT_KEY
        minimalAccount.transferOwnership(ANVIL_DEFAULT_ACCOUNT); 
        usdc = new ERC20Mock();
        vm.deal(address(minimalAccount), PAY_AMOUNT);
    }
    ...
    // You'll also need --system-mode=true to run this test - to call system contracts
    function testZkValidateTransaction() public {
        ...
        console2.log("Owner Account: ", minimalAccount.owner());
        console2.log("Minimal Account address: ", address(minimalAccount));
        console2.log("Sender address: ", address(uint160(transaction.from)));
        console2.log("Receiver address (USDC): ", address(uint160(transaction.to)));
    ...
    function _createUnsignedTransaction(address from, uint8 transactionType, address to, uint256 value, bytes memory data) internal view returns(Transaction memory) {
        ...
        return Transaction({
            ...
            //? Why type conversion? address are generally smaller & they dont fill up the 32 bytes, whereas uint256 fills up the the full 32 bytes
